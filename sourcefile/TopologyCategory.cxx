...
begin:
	...
	{
		//cout<< "goto i: "<< i << " vorder1 size "<< vorder1.size()<< "vorder2 size "<< vorder2.size() <<endl;
		if (vorder1.empty())
		{
			vorder1.push_back(i);
...
//      cout<< "goto i:"<<ngo<<" vorder1 size "<< vorder1.size()<< "vorder2 size "<< vorder2.size() <<endl;
//      cout<< "vorder1: ";
//      for (int i=0; i<vorder1.size();i++)	cout<<"\t"<<vorder1[i];	cout<<endl;	cout<< "vorder2: ";
//      for (int i=0; i<vorder2.size();i++)	cout<<"\t"<<vorder2[i];	cout<<endl;	cout<< "iChain: ";
//      for (int i=0; i<iChain.size();i++)	cout<<"\t"<<iChain[i];	cout<<endl;	cout<< "TopoChain: ";
//      for (int i=0; i<TopoChain.size();i++)	cout<<"\t"<<TopoChain[i];	cout<<endl;
//      if (ngo == 10 ) exit(0);
...
void TopologyCategory::openRootFile()
{
	...
	if (m_file == 0) exit(-1);
	if (m_tree == 0) exit(-2);
	//cout<<"Open file ..."<<m_file <<' '<<m_tree<<std::endl;
...
//	m_tree->SetBranchAddress("motheridx", motheridx);
	...
	m_tree->SetBranchAddress("pdgid", pdgid);
	m_tree->SetBranchAddress("motheridx", motheridx);
	//m_tree->SetBranchAddress("trkidx", trkidx);

//	m_tree->SetBranchAddress("indexmc_cm", &indexmc);
//	m_tree->SetBranchAddress("pdgid_cm", pdgid);
//	m_tree->SetBranchAddress("motheridx_cm", motheridx);

}

void TopologyCategory::TopoRead()
{
	cout<<"Topo Read ..."<<std::endl;
	...
	for(Int_t i=0; i<entriesAftCut; i++){
		if (i%1000 == 0) cout<<"process id "<< i <<endl;
		...
		{
			//if (j%2 == 1) continue;
			//std::cout<<"\tid: "<<j<<"\t"<<motheridx[j]<<"\t"<<pdgid[j]<<std::endl;
			int upidx = motheridx[j];
			if (upidx==j) upidx=-1;
			//if (motheridx[j]==trkidx[j]) upidx = -1;

			//midx.push_back(motheridx[j]);
			midx.push_back(upidx);
			//tidx.push_back(trkidx[j]);
		...
		{
			Vtopo.push_back(1);
			VtopoNpar.push_back(indexmc/2+1);
			VtopoStr.push_back(tempLine);
			n_itopo = totalTopo;
			for (int k=0; k<indexmc; k++)
			{
				//if (k%2 == 1) continue;
				int upidx = motheridx[k];
				if (upidx==k) upidx=-1;
				//if (motheridx[k]==trkidx[k]) upidx = -1;
