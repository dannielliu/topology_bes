...
def process(event):
    ...
    #   return ''
    i=0
    linel=[] # decay channels
    while i<100:  
       i+=1
       linel.append([]) 
    i=0
    line0=[] # final particles
    # recording particle from vpho
    #linel[0].append(-1)
    k=0
    j=0
    linel[0].append('-1')
    ...
    while (i<len(number)):
        ...
        # recording only final state particles and pi0s 
        # final state can be pi0
        if (number.count(mother[i]) >= 1 or mother[i]=='-1' ) : # be a daugther of anyone
           if (mother.count(number[i])==0): # has no daugther
              line0.append(pdgid[i])
        # for whole decay chain except pi0 decay
        if mother.count(number[i]) > 1 :  # has daughers/be a mother
           linel[k].append(pdgid[i])
           j=0
           while (j<len(number)):
    ...
    pline=''
    print 'length of line0 %i'%len(line0)
    for ii in line0:
       pline+=partex[ii]
    ...
    if ii.find("topology:") != -1:
       ...
       if imode%cutlines==0:
          if imode != 0:
             ...
             #h.write('\include{'+file1+'} '+' \\clearpage \n')
          file1='output/mclist_'+"%4.4i"%(imode/cutlines)
          f= open(file1+'.tex',"w")
          f.writelines(tab_head)
#
...
#h.write('\include{output/summary} '+'\\clearpage\n')
